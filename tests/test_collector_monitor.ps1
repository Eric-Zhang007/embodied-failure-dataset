...
try {
    $mainEpisode = [ordered]@{
        ...
        status = 'running'
        pid = 1205
        steps = @(@{ step_id = 'main__s0' }, @{ step_id = 'main__s1' })
    }
    $forkEpisode = [ordered]@{
    ...
    if ($snapshot.UnityCount -ne 2) {
        throw "Expected two Unity processes, got $($snapshot.UnityCount)."
    }
    ...
    if (($lines -join "`n") -notmatch 'Unity workers: 2') {
